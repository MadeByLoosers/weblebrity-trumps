...
WT.templates.playerTemplate = '<div class="player" id="player<%= number %>"><h2 class="score-heading">Player <%= number %>: <span class="score"><%= score %></span> card<span class="plural">s</span> remaining</h2><div class="card clearfix"></div></div>';
WT.templates.cardFront = '<div class="card-front"><h2><%= name %></h2><img src="https://api.twitter.com/1/users/profile_image?screen_name=<%= twitterName %>&size=original" height="160px"><% if (bio) { %><p class="bio"><%= bio %></p><% } %><ul></ul></div>';
